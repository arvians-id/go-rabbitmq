...
func (l *TodoLoader) Load(key int64) ([]*Todo, error) {
...
func (l *TodoLoader) LoadThunk(key int64) func() ([]*Todo, error) {
	l.mu.Lock()
...
func (l *TodoLoader) LoadAll(keys []int64) ([][]*Todo, []error) {
	results := make([]func() ([]*Todo, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	todos := make([][]*Todo, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *TodoLoader) LoadAllThunk(keys []int64) func() ([][]*Todo, []error) {
	results := make([]func() ([]*Todo, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *TodoLoader) Prime(key int64, value []*Todo) bool {
	l.mu.Lock()
	var found bool
	if _, found = l.cache[key]; !found {
		// make a copy when writing to the cache, its easy to pass a pointer in from a loop var
		// and end up with the whole cache pointing to the same value.
		cpy := make([]*Todo, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (l *TodoLoader) unsafeSet(key int64, value []*Todo) {
	if l.cache == nil {
		l.cache = map[int64][]*Todo{}
	}
	l.cache[key] = value
}
