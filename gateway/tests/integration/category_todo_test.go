...
var _ = Describe("CategoryTodo", func() {
	...
	Describe("Find all category todos", func() {
		When("The value of the data category todo is null", func() {
			It("Should return success with null data", func() {
				req := httptest.NewRequest(http.MethodGet, "/api/category-todos", nil)
		...
		When("The value of the data category todos isn't null", func() {
			It("Should return a success message upon successfully find all category todos", func() {
	...
	Describe("Find category todo by id", func() {
		When("The value of the data category todo is null", func() {
			It("Should throw an error not found", func() {
				req := httptest.NewRequest(http.MethodGet, "/api/category-todos/1", nil)
				resp, err := server.Test(req)
				Expect(err).NotTo(HaveOccurred())
		...
		When("The value of the data category todo isn't null", func() {
			It("Should return a success message upon successfully find category todo by id", func() {
				bodyRequest := strings.NewReader(`{"name": "Belajar Bahasa Pemrograman"}`)
				req := httptest.NewRequest(http.MethodPost, "/api/category-todos", bodyRequest)
				req.Header.Add("Content-Type", "application/json")
				resp, err := server.Test(req)
				...
				target := fmt.Sprintf("/api/category-todos/%d", id)
	...
	Describe("Create category todo", func() {
		Context("The data in the create category todo request is invalid", func() {
			When("The field name is blank", func() {
				It("Should throw a validation error in request with message 'bad request' on the 'name' field", func() {
					...
					Expect(responseBody["status"]).To(Equal("validation error on field:CategoryTodoCreateRequest.Name"))
					Expect(responseBody["data"]).To(BeNil())
				})
			})
		})

		When("The data in the create category todo request is valid", func() {
			It("Should return a success message upon successfully creating the category todo", func() {
				bodyRequest := strings.NewReader(`{"name": "Belajar Bahasa Pemrograman"}`)
				req := httptest.NewRequest(http.MethodPost, "/api/category-todos", bodyRequest)
				req.Header.Add("Content-Type", "application/json")
				resp, err := server.Test(req)
	...
	Describe("Delete category todo by id", func() {
		When("The value of the data category todo is null", func() {
			It("Should throw an error not found", func() {
				req := httptest.NewRequest(http.MethodDelete, "/api/category-todos/1", nil)
				resp, err := server.Test(req)
				Expect(err).NotTo(HaveOccurred())
